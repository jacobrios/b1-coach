...
**4. Taking the pen away works, and reaches less than you hope.** The three lessons above are one shape: the model holding a number it had no business holding. So the coach now writes a placeholder and the app substitutes the real figure. **Proven:** a value it hands over cannot be contradicted, and it cooperates about 89% of the time. **Estimated, never measured:** roughly a third of its errors gone, which is arithmetic over hand-checked data rather than a result. The more useful finding is what was left. A manual pass then caught the coach building its headline advice around a swing at 89 mph and 311 feet while a better one at 92 and 346 sat in the same target band. Every number was correct; it had chosen wrongly, which no number-handling fix reaches.
