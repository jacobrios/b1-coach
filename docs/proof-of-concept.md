...
**2. A handed number is safer, not safe.** The prompt said "Swings with exit velocity 85 mph or higher: 6 swings." The coach wrote "Six of your swings came in under 85 mph." The number was copied perfectly and the sentence inverted around it. The true answer was nine. Pre-counting cannot fix this, because the count was there and it was right, and neither does having the app write the digit, for the same reason.
...
**4. Taking the pen away works, and reaches less than you hope.** The three lessons above are one shape: the model holding a number it had no business holding. So I stopped letting it. The coach now writes a placeholder and the app substitutes the real figure, keeping the judgement and losing the typing. What that bought is worth separating carefully. **Proven:** a value the coach hands over cannot be contradicted, held by tests rather than by hope, and the coach cooperates with the mechanism about 89% of the time. **Estimated, and never measured:** roughly a third of its errors removed, which is arithmetic over hand-checked data and not a result. I did not buy the round that would have measured it, because the instrument in lesson 8 costs more to trust than the answer was worth. The deeper finding is what was left: **the residual is not transcription at all.** Within a day, a manual pass found the coach building its headline advice around the second-best swing, praising one at 89 mph and 311 feet while a better one at 92 and 346 sat in the same target band. Every number it printed was correct. It had chosen wrongly, which no amount of handing it numbers reaches, and which no tool I own can detect. That fix is a different shape: the app picks the swing and the coach is told to use it.

**5. Validate any model output that drives the interface, not just the words.** The model chooses which two charts to render by naming them. Twice, an invented name became a valid-looking object and failed silently, leaving an empty box where a chart should be. Names are now checked against the real list, duplicates dropped, and any rejected slot filled with a chart that works on real data. Model output selecting UI is a claim, exactly like model output describing a number.

**6. Tell the model what its numbers mean, and check the answer against the rest of the screen.** Spray direction reaches the coach as a signed number. For most of the project only one goal's instructions explained which sign meant pull, so on every other goal the coach inferred it, and during one review it called an opposite-field ball a pull-side ball. It was not hallucinating. It was reading data nobody had labelled. The first fix then shipped a *third* definition of pull, and manual review caught the coach naming six pull-side swings while the chart beside it coloured three. It was rejected and redone so that the prose and the chart read one shared definition. Labelling the data is half of it; making sure the label agrees with everything else the user can see is the other half.

**7. You cannot fix what you have not measured, and "it reads better" is not measurement.** Two separate attempts to shorten the coach by instruction alone appeared to work and quietly did not hold. Nothing noticed, because nothing was counting. The fix was not a better instruction, it was building something that could tell.
...
**10. Say what actually failed.** Every failure used to produce one sentence blaming a sleeping server, which was a guess presented as a fact and was often wrong. There are now four distinct messages: a drained API balance, a timeout, trouble at the API end, and an unreachable server. Two caveats worth stating rather than hiding: one of the four is also the catch-all, so it is not proof the API did anything wrong, and only two of the four have ever been forced against a real failure. On a demo, an honest failure costs far less trust than a confident wrong explanation, and that applies to how honestly you describe the failure handling too.

**The thread running through all ten: model output is a claim, not a fact.** That applies to a number in a sentence, a chart name that selects UI, and the output of the tool you built to check the model.
...
*Accuracy is improved in places, not solved.* Of the three error classes I aimed at, two are gone and one is reduced rather than eliminated: the coach's own groupings of pitch-location data went from six wrong claims to three across 52 debriefs, and the three that remain are all the same narrower failure, intersecting two groups it was handed separately. Lesson 4 is the decision I eventually took on the numbers themselves, and the cost I had spent months protecting against did not arrive: shown twelve blind pairs of debriefs, I could not tell which were the new ones. Lessons 2 and 3 still describe live failures with nothing shipped against them, and lesson 4's closing half is the one I would watch hardest, because the coach can be handed every number it needs and still choose the wrong one to talk about.
